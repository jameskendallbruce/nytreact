    ...
    <div className="card-body row mx-auto shadow-sm p-3 mb-5 bg-white rounded">
      <div className="col">
        <div className="card-header bg-light rounded"> 
          ...
          <span className="card-text">By {props.author}</span><span className="card-text"> || {props.date}</span>
        </div>
      </div>
      <div className="col">
        <p className="card-text ml-4"><i>{props.synopsis}</i></p>
        <button className="btn btn-outline-secondary ml-3" onClick={()=>props.onClick()} data-title={props.title} data-link={props.link} data-synopsis={props.synopsis} data-author={props.author}>Save</button>
      </div>
